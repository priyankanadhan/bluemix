...
class EmployeeModel {
	...
	public function getExport($colums,$regionId, $storeId, $employee, $userId){
		//print_r($colums);exit;
		$joins = '';
		$search_vale = '';
		$condition = '';
		$joins = "LEFT JOIN employee_role_store_designation on employee.id=employee_role_store_designation.employee_id
		LEFT JOIN designation on employee_role_store_designation.designation_id=designation.designation_id
		LEFT JOIN role on employee_role_store_designation.role_id=role.role_id
		...
		from employee
		$joins
		Where $condition group by employee.employees_id";
		
		/*$sql="select DISTINCT employee.id,$colums
		from employee
