...
class CustomersModel {
	...
	public function getExport($columns, $regionId, $storeId, $employee, $userId) {
		$joins = '';
		$condition = '';
		if ($_SESSION ['role'] == "1") {
			if ($regionId != "") {
				$joins = "LEFT JOIN product_category on customer.product_subcategory_id=product_category.product_category_id
		 				  LEFT JOIN product on product.product_category_id=product_category.product_category_id
		                  LEFT JOIN employee on customer.last_updated_by=employee.id
		                  LEFT JOIN employee_role_store_designation on customer.last_updated_by=employee_role_store_designation.employee_id
		                  LEFT JOIN store on store.store_id=employee_role_store_designation.store_id
		                  LEFT JOIN store_region on store_region.store_region_id=store.store_region_id ";
				if ($employee != "" && $regionId != "") {
					$condition = 'employee.id="' . $employee . '" and store_region.store_region_id="' . $regionId . '"';
				} elseif ($employee != "" && $regionId != "" && $storeId != "") {
					$condition = 'employee.id="' . $employee . '" and store_region.store_region_id="' . $regionId . '" and store.store_id="' . $storeId . '"';
				} elseif ($regionId != "" && $storeId != "") {
					$condition = 'store_region.store_region_id="' . $regionId . '" and store.store_id="' . $storeId . '"';
				} elseif ($regionId != "") {
					$condition = 'store_region.store_region_id="' . $regionId . '"';
				} else {
					$condition = '1';
				}
			} else {
			...
			if ($employee != "") {
				...
				$condition = 'customer.store_id="' . $_SESSION ['store'] . '"';
			} else {
				$condition = '1';
			}
		} elseif ($_SESSION ['role'] == "3") {
			$joins = "LEFT JOIN product_category on customer.product_subcategory_id=product_category.product_category_id";
			$condition = "customer.lead_status='open' and customer.last_updated_by = '" . $userId . "'";
		...
		FROM customer
		$joins
		WHERE $condition group by customer.customer_id";
		...
		foreach ( $customers as $customer ) {
			// print_r($customer->customer_id);exit;
			// $customer='948';
			$selectComments = "select customer_comments as Comments2
					           from customer_history 
					           where customer_id='" . $customer->customer_id . "'";
			$query1 = $this->db->prepare ( $selectComments );
			// $parameters = array(':start' => $start,':limit'=> $limit);
			$query1->execute (); // $parameters);
			                     // fetchAll() is the PDO method that gets all result rows, here in object-style because we defined this in
			
			$cusComments = $query1->fetchAll ();
			$customer->Comments2 = $cusComments;
			$dataArray [] = $customer;
		}
		return $dataArray;
	}
	public function getEmployeesByStoreId($storeId) {
		$sql = "SELECT name as employee_name,
				id as employee_id from employee 
	...
	public function getAllCustomerLeads($regionId, $storeId, $employee, $start, $limit, $searchKey, $orderStr, $userId, $lead_type, $lead_status, $demo) {
		$colums = '';
		$joins = '';
		$search_vale = '';
		$condition = '';
		if ($_SESSION ['role'] == "1") {
			...
			if ($regionId != "") {
				...
				if ($lead_type != "" && $lead_status != "" && $demo) {
					$condition .= 'and c.lead_type="' . $lead_type . '" AND c.lead_status="' . $lead_status . '" AND c.cust_demo="' . $demo . '"';
				} elseif ($lead_type != "" && $lead_status != "") {
					$condition .= 'and c.lead_type="' . $lead_type . '" AND c.lead_status="' . $lead_status . '"';
				} elseif ($lead_type != "") {
					$condition .= 'and c.lead_type="' . $lead_type . '"';
				} elseif ($lead_status != "") {
					$condition .= 'and c.lead_status="' . $lead_status . '"';
				} elseif ($demo != "") {
					$condition .= 'and c.cust_demo="' . $demo . '"';
				}
			} else {
				$joins = "LEFT JOIN product_category as pc on c.product_subcategory_id=pc.product_category_id
					LEFT JOIN employee as e on c.last_updated_by=e.id
		    		...
		    		LEFT JOIN store_region as sr on sr.store_region_id=s.store_region_id";
				if ($lead_type != "" && $lead_status != "" && $demo) {
					$condition = 'c.lead_type="' . $lead_type . '" AND c.lead_status="' . $lead_status . '" AND c.cust_demo="' . $demo . '"';
				} elseif ($lead_type != "" && $lead_status != "") {
					$condition = 'c.lead_type="' . $lead_type . '" AND c.lead_status="' . $lead_status . '"';
				} elseif ($lead_type != "") {
					$condition = 'c.lead_type="' . $lead_type . '"';
				} elseif ($lead_status != "") {
					$condition = 'c.lead_status="' . $lead_status . '"';
				} elseif ($demo != "") {
					$condition = 'c.cust_demo="' . $demo . '"';
				} else {
					$condition = '1';
				}
			}
	 ...
	 */
	public function getCustomersCount($regionId, $storeId, $employee, $searchKey = '', $userId, $lead_type, $lead_status, $demo) {
		if ($_SESSION ['role'] == "1") {
			$colums = "sr.store_region_name,s.store_name,e.name as employee_name,e.employees_id,c.customer_id,c.lead_status, c.firstname, c.email,c.phone,pc.product_category_name,c.product_id,DATE_FORMAT(c.last_updated_date,'%d-%m-%Y') as last_updated_date,
    				DATE_FORMAT(c.next_followup,'%d-%m-%Y') as next_followup";
			$search_vale = "e.name LIKE '%" . $searchKey . "%' OR e.employees_id LIKE '%" . $searchKey . "%' OR c.lead_status LIKE '%" . $searchKey . "%' OR s.store_name LIKE '%" . $searchKey . "%' OR sr.store_region_name LIKE '%" . $searchKey . "%' OR c.firstname LIKE '%" . $searchKey . "%' OR c.phone LIKE '%" . $searchKey . "%' OR c.`next_followup` LIKE '%" . $searchKey . "%' OR pc.`product_category_name` LIKE '%" . $searchKey . "%'";
			if ($regionId != "") {
				...
				if ($lead_type != "" && $lead_status != "") {
					$condition .= 'and c.lead_type="' . $lead_type . '" AND c.lead_status="' . $lead_status . '"';
				} elseif ($lead_type != "" && $lead_status != "" && $demo) {
					$condition .= 'and c.lead_type="' . $lead_type . '" AND c.lead_status="' . $lead_status . '" AND c.demo="' . $demo . '"';
				} elseif ($lead_type != "") {
					$condition .= 'and c.lead_type="' . $lead_type . '"';
				} elseif ($lead_status != "") {
					$condition .= 'and c.lead_status="' . $lead_status . '"';
				...
				if ($lead_type != "" && $lead_status != "") {
					...
					$condition = 'c.lead_type="' . $lead_type . '" AND c.lead_status="' . $lead_status . '" AND c.demo="' . $demo . '"';
				} elseif ($lead_type != "") {
					$condition = 'c.lead_type="' . $lead_type . '"';
				} elseif ($lead_status != "") {
					$condition = 'c.lead_status="' . $lead_status . '"';
	...
	public function updateCustomerDetailsByAdmin($customer_array, $customersId) {
		$sql = "UPDATE  `customer` SET
                    `firstname`='" . $customer_array ['firstname'] . "',
					 ...
					 product_id='" . $customer_array ['product_name'] . "'
					 where customer_id='" . $customersId . "'";
	...
	public function updateAdditionalDetailsByAdmin($cust_demo, $cust_source, $lead_type, $customersId) {
		$sql = "UPDATE `customer`
    				SET `cust_demo` ='" . $cust_demo . "',
    					`cust_source` = '" . $cust_source . "',
    					`lead_type` = '" . $lead_type . "'
	 ...
	 * Service for app
	 */
	public function getTokenAuthendication($token) {
		$sql = "SELECT * FROM token WHERE token = :token LIMIT 1";
		$query = $this->db->prepare ( $sql );
		$parameters = array (
				':token' => $token 
		);
	...
	public function getUserDetailsByToken($userId) {
		$sql = "SELECT employee_role_store_designation.*,store.store_id,store_region.store_region_id
					...
					where employee_id = :userid and employee_role_store_designation.active_status = '1' LIMIT 1";
	...
	public function createCustomer($customer_array) {
		...
		$parameters = array (
				...
				':frm_product_subcategory_id' => $customer_array ['product_subcategory_id'],
				':frm_product_id' => $customer_array ['product_id'],
				':ses_store_id' => $customer_array ['store_id'],
				':ses_userid' => $customer_array ['user_id']
		);
		// useful for debugging: you can see the SQL behind above construction by using:
		// echo '[ PDO DEBUG ]: ' . Helper::debugPDO($sql, $parameters); exit();
		// echo $sql;exit;
